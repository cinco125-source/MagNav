##* Does proper resampling (thresh=0.5) reduce the particle count needed?
#
# At thresh=0.1 the repaired native MPF needed N=12000 to reach the EKF on 1003.08.
# The resample sweep showed thresh=0.5 is the best of {0.1,0.3,0.5,0.7} at N=1000
# (1003: 65.5, 1007: 85.4 -- better than 0.1's 109/204 but still above EKF 41/33).
# Here we re-run the particle-count ladder at thresh=0.5 to see whether standard
# resampling brings the EKF-class point down from N=12000 to a smaller N.
#
# offline-NN Mag 5, matched R=40 nT, roughen 5 m, N in {1000, 4000}. EKF reference.
# Outputs research/mpf_np_t05_results.csv. Usage: julia --project=. research/mpf_np_t05.jl
...
MEAS_STD = 40.0
ROUGH    = 5.0
THR      = 0.5      # standard resampling (ESS < N/2)
NPS      = [1000, 4000]
...
for (fl,line) in LINES
    ...
    println("\n$fl $line ($mname)  EKF=$(round(d_ekf,digits=1)) m  (thresh=$THR)")
...
CSV.write(joinpath(@__DIR__,"mpf_np_t05_results.csv"), results)
println("\n=== particle ladder at thresh=0.5: offline-NN Mag 5 ===")
show(results, allrows=true, allcols=true); println()
println("wrote mpf_np_t05_results.csv")
