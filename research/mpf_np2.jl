##* Particle-count convergence, continued: push the still-descending line to N=8000/12000.
#
# mpf_np.jl showed MPF -> EKF as N grows: 1007.06 already converged at N=4000
# (MPF 32.9 = EKF 33.3), but 1003.08 was still descending (N=1000: 109, N=4000: 68.7,
# EKF 41.2). This extends ONLY 1003.08 to N in {8000, 12000} to complete its curve.
# 1007.06 is omitted (already at its EKF plateau) to fit the time budget.
#
# Matched config (R=40 nT ~ offline-NN residual, roughen 5 m), offline-NN Mag 5.
# Outputs research/mpf_np2_results.csv. Usage: julia --project=. research/mpf_np2.jl
...
ROUGH    = 5.0
NPS      = [8000, 12000]
LINES = [(:Flt1003,1003.08)]        # only the still-descending line
...
CSV.write(joinpath(@__DIR__,"mpf_np2_results.csv"), results)
println("\n=== particle-count convergence (1003.08 extended): offline-NN Mag 5 ===")
show(results, allrows=true, allcols=true); println()
println("wrote mpf_np2_results.csv")
