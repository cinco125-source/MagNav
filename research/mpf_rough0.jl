##* Is roughening necessary at standard resampling, or can we drop it?
#
# thresh=0.5 (standard ESS<N/2) + roughen=5 reached EKF-class at N=4000 (1003: 36.3,
# 1007: 35.1). Roughening is an add-on beyond the standard bootstrap RBPF (Canciani has
# none), so a cleaner baseline drops it IF performance holds. This runs the same config
# with roughen=0: if N=4000 still reaches EKF-class, the baseline is pure standard RBPF
# (bootstrap proposal + ESS<N/2 resampling + R matched to residual), no regularization.
#
# offline-NN Mag 5, matched R=40 nT, thresh=0.5, roughen=0, N in {1000,4000}. EKF ref.
# Outputs research/mpf_rough0_results.csv. Usage: julia --project=. research/mpf_rough0.jl
...
FOGM_TAU = 180.0
MEAS_STD = 40.0
ROUGH    = 0.0      # NO roughening -- pure standard RBPF
THR      = 0.5      # standard ESS<N/2 resampling
NPS      = [1000, 4000]
...
results = DataFrame(flight=Symbol[],line=Float64[],num_part=Int[],thresh=Float64[],
                    roughen_m=Float64[],EKF=Float64[],MPF=Float64[],t_s=Float64[])
...
for (fl,line) in LINES
    ...
    println("\n$fl $line ($mname)  EKF=$(round(d_ekf,digits=1)) m  (thresh=$THR, roughen=$ROUGH)")
    ...
    for np in NPS
        ...
        push!(results,(fl,line,np,THR,ROUGH,round(d_ekf,digits=1),round(d_mpf,digits=1),round(t,digits=1)))
        println("  N=$np   MPF=$(round(d_mpf,digits=1)) m   ($(round(t,digits=0)) s)")
    end
end

CSV.write(joinpath(@__DIR__,"mpf_rough0_results.csv"), results)
println("\n=== standard resampling, NO roughening: offline-NN Mag 5 ===")
show(results, allrows=true, allcols=true); println()
println("wrote mpf_rough0_results.csv")
