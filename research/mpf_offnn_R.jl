##* R-matched retry: is the offline-NN MPF divergence config (R over-confidence /
#* roughening) or fundamental PF fragility?
#
# Theory says the marginalized PF should upper-bound the EKF (particles handle the
# nonlinear map term with no linearization, the rest is exact Kalman), so MPF worse
# than EKF on the SAME accurate measurement points at configuration, not the method.
# The prime suspects:
#   (1) R over-confidence: mpf_offnn set meas_var = 12^2 (12 nT), but the offline-NN
#       residual is ~36-50 nT. A too-small R makes the PF likelihood far too peaked
#       -> weight collapse -> depletion. The EKF tolerates the same R mismatch.
#   (2) roughening adds a position error floor if the filter is otherwise healthy.
#
# Sweep meas_var x roughening for the repaired native MPF on the offline-NN
# compensated Mag 5 (the accurate signal, EKF ~29-39 m), with an EKF at the matched R
# as reference. If a matched R (and/or roughen 0) recovers MPF -> config. If it still
# diverges everywhere -> fundamental.
#
# Outputs research/mpf_offnn_R_results.csv. Usage: julia --project=. research/mpf_offnn_R.jl
...
NP       = 1000
THR      = 0.1
MEAS_STDS = [12.0, 25.0, 40.0, 80.0]   # measurement-noise std sweep [nT]
ROUGHS    = [0.0, 5.0]                  # roughening sweep [m]
...
##* train one frozen offline :m1 model for Mag 5 (the accurate cabin mag) ---------
...
results = DataFrame(flight=Symbol[],line=Float64[],comp_rmse_nT=Float64[],
                    meas_std_nT=Float64[],roughen_m=Float64[],
                    EKF=Float64[],MPF=Float64[])
...
for (fl,line) in LINES
    ...
    println("\n$fl $line ($mname)  offNN comp-RMSE = $(round(crms,digits=1)) nT")

    for ms in MEAS_STDS
        mv = ms^2
        (P0,Qd,R) = create_model(traj.dt,traj.lat[1];init_pos_sigma=3.0,init_alt_sigma=1.0,
            init_vel_sigma=1.0,meas_var=mv,fogm_sigma=FOGM_SIG,fogm_tau=FOGM_TAU)
        d_ekf = try
        ...
        for rgh in ROUGHS
            ...
            push!(results,(fl,line,round(crms,digits=1),ms,rgh,
                           round(d_ekf,digits=1),round(d_mpf,digits=1)))
            println("  meas_std=$(ms) nT  roughen=$(rgh) m   EKF=$(round(d_ekf,digits=1))  MPF=$(round(d_mpf,digits=1)) m")
        end
    end
end

CSV.write(joinpath(@__DIR__,"mpf_offnn_R_results.csv"), results)
println("\n=== R-matched sweep: offline-NN Mag 5, EKF vs repaired native MPF ===")
show(results, allrows=true, allcols=true); println()
println("wrote mpf_offnn_R_results.csv")
