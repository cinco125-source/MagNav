##* Change ONLY the resampling schedule: Canciani-style every-step (thresh=1.0).
#
# Canciani & Raquet resample at EVERY step (SIR). Our diagnostics used thresh=0.1
# (resample only when ESS<0.1N, i.e. rarely) -- the one real deviation from the
# reference MPF. Here we flip resampling to every-step (thresh=1.0: ESS<N is almost
# always true) and change NOTHING else (offline-NN Mag 5, matched R=40 nT, roughen 5),
# to see whether reference-faithful resampling reaches EKF-class at fewer particles.
#
# N in {1000, 4000}, both lines, EKF reference.
# Outputs research/mpf_every_results.csv. Usage: julia --project=. research/mpf_every.jl
...
MEAS_STD = 40.0
ROUGH    = 5.0
THR      = 1.0      # every-step resampling (ESS<N almost always) = Canciani SIR
NPS      = [1000, 4000]
...
for (fl,line) in LINES
    ...
    println("\n$fl $line ($mname)  EKF=$(round(d_ekf,digits=1)) m  (every-step resample, thresh=$THR)")
...
CSV.write(joinpath(@__DIR__,"mpf_every_results.csv"), results)
println("\n=== every-step resampling (Canciani SIR): offline-NN Mag 5 ===")
show(results, allrows=true, allcols=true); println()
println("wrote mpf_every_results.csv")
