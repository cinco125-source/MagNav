##* Is the huge particle count a resampling problem, not a fundamental need?
#
# 1003.08 needed N=12000 to reach the EKF (39.5 vs 41.2). For a 2D position that is a
# lot of particles -- a red flag for particle inefficiency. The prime suspect is the
# resample threshold: mpf_logw resamples only when ESS < thresh*N, and we ran thresh=0.1
# (resample almost never) -> weight degeneracy accumulates -> few effective particles ->
# need many more. Standard practice is thresh~0.5 (ESS < N/2).
#
# Fix N=1000 (where MPF was 109/203, well above EKF) and sweep the resample threshold.
# If a proper threshold (0.5) brings N=1000 to EKF-class, the 12000 was a resampling
# artifact, not a real particle requirement. offline-NN Mag 5, matched R=40 nT, roughen 5.
#
# Outputs research/mpf_resamp_results.csv. Usage: julia --project=. research/mpf_resamp.jl
...
results = DataFrame(flight=Symbol[],line=Float64[],num_part=Int[],thresh=Float64[],
                    EKF=Float64[],MPF=Float64[],t_s=Float64[])
...
for (fl,line) in LINES
    ...
    println("\n$fl $line ($mname)  EKF=$(round(d_ekf,digits=1)) m  (N=$NP)")

    for thr in THRS
        t = @elapsed (d_mpf = try
            fr = mpf_logw(ins,mag_nn,itp_lin;P0=P0,Qd=Qd,R=R,num_part=NP,
                          thresh=thr,roughen_m=ROUGH,core=true)
            fr.c ? drms_ll(traj,(fo=MagNav.eval_filt(traj,ins,fr)).lat,fo.lon) : Inf
        catch e; @warn("mpf_logw",e); Inf end)
        push!(results,(fl,line,NP,thr,round(d_ekf,digits=1),round(d_mpf,digits=1),round(t,digits=1)))
        println("  thresh=$thr (resample when ESS<$(thr)N)   MPF=$(round(d_mpf,digits=1)) m   ($(round(t,digits=0)) s)")
    end
end

CSV.write(joinpath(@__DIR__,"mpf_resamp_results.csv"), results)
println("\n=== resample-threshold sweep at N=1000: offline-NN Mag 5 ===")
show(results, allrows=true, allcols=true); println()
println("wrote mpf_resamp_results.csv")
