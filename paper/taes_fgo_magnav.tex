...
and $c_t$ is the aircraft interference, decomposed below into a Tolles--Lawson part
and a first-order Gauss--Markov remainder $S_t$. Linearizing about the nominal position,
\begin{equation}
...
best of the four on all eight full-length cases; under a paired sign test on the
...
smoother's \SI{13.1}{\meter} (Table~\ref{tab:breadth}), and on the first ten-minute
segment \SI{17.5}{} against \SI{21.4}{\meter}, the GTSAM side slightly the better of
the two in both cases; the free-inertial drift matches at \SI{317.5}{} against
...
line and both runs are single, so this is a statement about implementation
...
every step. Through the weakly observable first minutes that estimate is poor: counted
from \SI{60}{\second} onward, the causal DRMS is \SI{82.4}{\meter} from a cold
Tolles--Lawson start and \SI{49.5}{\meter} with a warm prior, the transient of
Section~\ref{sec:observability}. Once the smoother locks on, counted from
\SI{300}{\second} onward, the \SI{30}{\second}-lag causal DRMS is \SI{16.5}{\meter}
cold and \SI{15.1}{\meter} warm, against \SI{11.0}{\meter} for the
\SI{300}{\second}-lag smoothed estimate over the same interval and far below the
free-inertial drift (Fig.~\ref{fig:realtime}).
...
minutes and converges toward the \SI{300}{\second}-lag smoothed estimate, far below
free-inertial drift.}
\label{fig:realtime}
\end{figure}

The lag behaves as Section~\ref{sec:method} predicts. Lengthening it improves the
committed estimate, from \SI{13.4}{\meter} smoothed at a \SI{30}{\second} lag to
\SI{11.0}{\meter} at a \SI{300}{\second} lag (both counted from \SI{300}{\second}
onward), but at a cost our unoptimized Python callbacks cannot yet meet in real time
at the long lag, so that setting is presently offline in this implementation; native
factors would move it online. Shortening the lag to \SI{30}{\second} restores real
time at the price of the cold-start transient analyzed in
Section~\ref{sec:observability}: a warm compensation prior halves that transient, and
lag scheduling, running a long lag through the early, weakly observable minutes and
contracting once the compensation has converged, would address the rest; we have not
yet evaluated such a schedule.
