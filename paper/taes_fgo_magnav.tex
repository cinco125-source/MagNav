...
the forward-filtered state or marginalize the overlap by a Schur
complement~\cite{leutenegger2015,kaess2012}; we accept the bounded
optimism (the overlap is a fixed fraction, $L_o/L_w\!=\!0.3$, of each window) in
exchange for the simpler implementation. The reported DRMS is a comparison of
means: Table~\ref{tab:overlap} shows it is empirically insensitive to $L_o$, so the
double-count affects only the covariance, not the point estimate.
...
$L_o$. Table~\ref{tab:overlap} sweeps $L_o$ from $0$ to \SI{150}{\second}: the DRMS is
insensitive to the choice, and the no-overlap handoff ($L_o=0$), which carries no
future data and incurs no double-count, is in fact the worst or near-worst on every
case, while adding look-ahead lowers the mean by up to \SI{9}{\meter}. The
information double-count noted in Sec.~\ref{sec:method}-B is therefore benign for the
point estimate---the look-ahead it buys outweighs the mild measurement
reuse---so an exactly consistent handoff would not improve the reported DRMS. The
\SI{90}{\second} column reproduces the operating-point numbers used throughout
(Table~\ref{tab:breadth}). In the experiments we use $L_w=\SI{5}{\minute}$ and
$L_o=\SI{90}{\second}$; the
...
\begin{table}[t]
\caption{Overlap $L_o$ versus horizontal DRMS [m] at $L_w=\SI{5}{\minute}$, cold
start, uncompensated cabin magnetometers. $L_o=0$ removes both the look-ahead and the
overlap double-count; $L_o=\SI{90}{\second}$ (bold) is the operating point used
throughout and reproduces Table~\ref{tab:breadth}.}
\label{tab:overlap}
\centering
\begin{tabular}{llrrrrrr}
\toprule
 & & \multicolumn{6}{c}{Overlap $L_o$ [s]} \\
\cmidrule(l){3-8}
Line & Mag & 0 & 30 & 60 & \textbf{90} & 120 & 150 \\
\midrule
1007.06 & 4 & 34.6 & 30.1 & 35.8 & \textbf{32.7} & 29.7 & 30.3 \\
        & 5 & 14.5 & 13.5 & 13.9 & \textbf{13.8} & 13.0 & 13.6 \\
1003.08 & 4 & 35.4 & 31.0 & 32.1 & \textbf{26.1} & 26.4 & 29.5 \\
        & 5 & 15.7 & 14.0 & 13.0 & \textbf{12.4} & 12.5 & 12.1 \\
\bottomrule
\end{tabular}
\end{table}
...
the NN-augmented filter on both sensors, \SI{16.2}{\meter} on Mag~4 and
\SI{4.1}{\meter} on Mag~5, without any neural network, using only the linear TL
