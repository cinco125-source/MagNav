...
means: Table~\ref{tab:overlap} indicates it varies little with $L_o$, so the
double-count primarily affects the covariance rather than the point estimate.
...
$L_o$. Table~\ref{tab:overlap} sweeps $L_o$ from $0$ to \SI{150}{\second}: the DRMS varies
little across this range, and the no-overlap handoff ($L_o=0$), which carries no
future data and incurs no double-count, is among the worst on each case, with the
look-ahead configurations somewhat lower. This suggests the information double-count
noted in Sec.~\ref{sec:method}-B has little effect on the point estimate over the
cases examined; an exactly consistent handoff would mainly affect the covariance
rather than the reported DRMS. The \SI{90}{\second} column reproduces the
operating-point numbers used throughout (Table~\ref{tab:breadth}). In the experiments we use $L_w=\SI{5}{\minute}$ and
