...
% P2: Scope Narrowing -- the problem, stated generally
...
absolute fix that bounds inertial drift~\cite{canciani2017}. Its central obstacle
is not the map but the aircraft itself: the platform's own permanent and induced
magnetization and its eddy-current fields perturb the scalar measurement by
hundreds of nanotesla, comparable to or larger than the anomaly signal being
matched, so the aircraft field must be compensated before map matching can
recover position. This aircraft-field problem is intrinsic to MagNav and
independent of any particular airframe; it is most acute for magnetometers
mounted inside the cabin, close to avionics and current-carrying structure,
rather than on a wingtip stinger boom that holds the sensor away from the platform
field.
...
position, a coupling that makes both estimation and its analysis substantially
harder than the calibrated case.

A public benchmark has made these difficulties concrete and measurable: the Signal
Enhancement for Magnetic Navigation (SGL~2020) challenge dataset~\cite{gnadt2020},
comprising flight-grade INS, multiple onboard magnetometers (both wingtip-stinger
and cabin-mounted) and high-resolution anomaly maps, which has catalyzed most
recent work on the problem. A second difficulty, prominent precisely on its cabin
magnetometers, is that the linear TL basis does not span the true interference:
platform currents and other non-rigid, non-attitude sources contribute a large
fraction of the field that no set of Tolles--Lawson
coefficients can represent~\cite{gnadt2022}. This has motivated a line of
learning-augmented compensation on this dataset: neural-network heading-error and
denoising models~\cite{williams1993,gnadt2022}, physics-informed
liquid-time-constant networks~\cite{nerrise2024}, and the online EKF+TL+NN whose
residual network mops up exactly this un-modeled remainder~\cite{hager2026}. The
scalar optically-pumped magnetometers used for the cabin channels add their own
orientation-dependent errors (heading error and equatorial dead
zones~\cite{hager2026b,wang2020}) on top of the platform field. How to absorb the
non-TL remainder without a neural network is one of the questions this paper
addresses.
...
problem as a fixed-lag sliding-window smoother (Fig.~\ref{fig:concept}). The
per-window relinearization
...
\begin{enumerate}
  \item \textbf{Neural-network-free online compensation.} We formulate online
        Tolles--Lawson compensation as a chain of time-varying variables in a
        fixed-lag sliding-window factor graph and solve navigation and
        compensation jointly. The per-window relinearization supplies the same
        time-varying compensation that the competing method draws from an online
        network, while deferring commitment over the window removes the causal
        filter's cold-start failure mode. The result is an estimator that, on the
        uncompensated cabin magnetometers, matches or beats the online EKF+TL+NN
        of~\cite{hager2026} without any learned component (Section~\ref{sec:method},
        Table~\ref{tab:1007}). We are deliberate about what is and is not claimed:
        for the linear--Gaussian error model the batch estimate equals the RTS
        smoother, so smoothing per se is not the contribution: the joint,
        windowed estimation of a time-varying compensation together with
        the robustness it buys at cold start is.
  ...
  \item \textbf{Reimplemented baseline and multi-line breadth study.} We
        reimplement, rather than merely cite, the online EKF+TL+NN result of
        \cite{hager2026} and place it head-to-head with the proposed smoother, a
        weak online-TL EKF, and a marginalized particle filter carrying the TL
        coefficients, on eight cold-start cases spanning four long lines, two
        flights, and two anomaly maps. The study isolates exactly what the neural
        network buys (it keeps the causal filter bounded where online TL alone and
        the particle filter both diverge) and what it does not (on every full-length
        navigation line the NN-free smoother is the most accurate of the four),
        giving a like-for-like comparison that the literature has lacked
...
structure, not their noise models. Under it the batch estimate coincides with
...
not: jointly estimating the time-varying $\boldsymbol{\beta}$, adding the
...
for the Gaussian case and we verify the two agree to within a metre on a real-data
segment in Section~\ref{sec:results} (the check covers the navigation-only chain;
the joint TL problem is solved by the iterated-RTS path).
...
\begin{corollary}[Exogeneity and separability]\label{cor:exo}
Suppose $\mathbf{A}_t$ is a function of aircraft attitude/fluxgate
alone, exogenous to position. Then no column of $\boldsymbol{\Psi}$ is generated
by the map gradient along the trajectory, and condition (iii) holds for
generic trajectories and maps (in particular, it requires the window to satisfy
$N\ge\dim\mathrm{range}(\mathbf{G})+\dim\mathrm{range}(\boldsymbol{\Psi})$ and no
$\boldsymbol{\Psi}$ column to be a linear combination of the map-gradient time
series). Position--compensation separability is then independent of the
basis dimension: enlarging an exogenous basis can slow the coefficient estimate
through (ii) but cannot mask position. If instead a compensation feature
contains the map value $h(\mathbf{p})$ (for example the scalar magnetometer
itself), its column lies in $\mathrm{range}(\mathbf{G})$ by construction, forcing
the intersection nonzero and collapsing observability.
\end{corollary}

\begin{remark}[TL rank deficiency as gauge freedom]\label{rem:gauge}
Condition (ii) fails for the classical TL basis itself: with unit direction
cosines, the three induced diagonal columns sum to
$B_t(u^2{+}v^2{+}w^2)=B_t$, collinear with the constant-bias column, the
long-known collinearity of TL regression~\cite{leliak1961,noriega2015}. This is a
gauge freedom of the compensation, not a navigation defect: any
$\delta\boldsymbol{\beta}\in\ker\boldsymbol{\Psi}$ leaves
$c_t=\mathbf{A}_t^{\top}\boldsymbol{\beta}_t$, and hence every prediction and the
position estimate, unchanged. The prior $\mathbf{P}_0$ and the random-walk factors
select one representative of each gauge orbit. Separability of position from
compensation is governed by (iii) alone and is unaffected.
\end{remark}
...
observe on the shortest line in Section~\ref{sec:results}. Second, an endogenous feature such as the scalar
...
accuracy conclusion from it: in particular the NN's \SI{105.2}{\meter} on its Mag~5,
...
\midrule
1007.06 & FF & R & 0.4 & 4 & 46.7 & 48.8 & div. & \textbf{32.7} \\
...
1003.02 & SV & E & 0.4 & 4 & div. & 101.0 & div. & \textbf{42.6} \\
...
non-Gaussian representation. What the window adds is that it reaches the same bounded
...
pattern. Exogeneity (Corollary~\ref{cor:exo}) is why the scalar magnetometer had to
be excluded from the network inputs for the baseline to converge, and why the
attitude-only TL basis stays safe at any dimension. The map-information floor of the
...
factor-graph variables in a fixed-lag sliding-window smoother yields a cold-start
calibrator that uses no neural network. On real SGL~2020 data it matches or beats a
reimplemented online EKF+TL+NN on the primary free-flight line and generalizes
across four lines, two flights, and two maps, staying bounded where the causal EKF
diverges. An exact observability condition and its exogeneity corollary explain this
behaviour and guide the choice of compensation features.

Several limitations bound the study and set the agenda for further work. Each real
line is a single physical realization, so its DRMS is a point estimate; a simulation
Monte-Carlo (Section~\ref{sec:results}-F) establishes covariance consistency and
statistical significance, but repeating the real lines and extending to the full SGL
line set remain out of reach with a single recorded flight per line. The comparison
covers a causal online-TL EKF, a reimplemented online EKF+TL+NN, and a marginalized
particle filter carrying the TL coefficients; a like-for-like grid or point-mass
MMSE map-matching baseline~\cite{bergman1999,nordlund2009} would further sharpen the
claim about estimator structure. The sensor-error factors are validated in simulation only, and
a flight demonstration with genuine optically-pumped-magnetometer heading error is
still needed. Most important, although the primary line comes from the free-flight
mission and is the community's navigation benchmark, none of the SGL~2020 lines is a
dedicated long, straight-and-level operational transit; their attitude content both
stresses compensation and, by exciting the TL regressor, aids its observability, so
performance on a pure operational cruise with little attitude variation is not
established here and is the natural next step. Short lines remain hard for every
method, which is an information limit rather than an estimator one. None of these
qualifications affects the central and reproducible finding, that a linear smoother
with no neural network is the best of four cold-start estimators on all eight
full-length cases, bounded where both the plain causal filter and a marginalized
particle filter diverge, and more accurate than an NN-augmented causal filter that
does use one.
