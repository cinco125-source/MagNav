...
Given the scalar measurements $z_{1:N}$ (Section~\ref{sec:background}-B), the
fluxgate regressor rows $\mathbf{A}_{1:N}$ (Section~\ref{sec:background}-C), the
anomaly map $h$, the INS mechanization (Section~\ref{sec:background}-A), and the
...
Problem~\ref{prob:joint} concrete---how the navigation error evolves
(Section~\ref{sec:background}-A), what the scalar magnetometer measures
(Section~\ref{sec:background}-B), and how the aircraft field enters it
(Section~\ref{sec:background}-C)---and then casts the whole as maximum-a-posteriori
estimation on a factor graph (Section~\ref{sec:background}-D).
...
maximum-a-posteriori (MAP) estimate of Problem~\ref{prob:joint}. By the Markov structure of the
...
(Table~\ref{tab:1007}), consistent with the cold-start operating point published
for this line in~\cite{hager2026}---a functioning but not exact reimplementation,
which we use as the baseline. All numbers compared in
Section~\ref{sec:results} are produced by our own runs, under one protocol.
...
\num{40.0}/\SI{17.5}{\meter} (Section~\ref{sec:baseline} validates the
reimplementation against the published operating point). The window smoother beats
the NN-augmented filter on both sensors---\SI{7.4}{\meter} on Mag~4 and
\SI{3.3}{\meter} on Mag~5---without any neural network, using only the linear TL
basis carried as graph variables. All compared numbers are our own runs under one
protocol; 1007.06 is the line on which the reimplementation is anchored, and the
...
NEES divides each estimator's error by its \emph{own} reported covariance, so a
...
reported uncertainty, so this checks credibility, not accuracy).}
\label{fig:sigma}
\end{figure}
