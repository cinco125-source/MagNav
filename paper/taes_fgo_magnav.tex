...
\emph{prior} factor on $\boldsymbol{\chi}_1$; $N{-}1$ binary \emph{INS dynamics}
factors, each linking $\mathbf{x}_t$ to $\mathbf{x}_{t+1}$
through~\eqref{eq:dyn}; $N{-}1$ binary \emph{compensation random-walk} factors,
each linking $\boldsymbol{\beta}_t$ to $\boldsymbol{\beta}_{t+1}$; and $N$ unary
\emph{measurement} factors, each attached to one node $\boldsymbol{\chi}_t$
...
needs a condition (Section~\ref{sec:observability}). MAP estimation on such a graph is sparse nonlinear
...
\begin{remark}[Exogeneity design rule]\label{cor:exo}
When every compensation feature is exogenous---$\mathbf{A}_t$ a function of aircraft
attitude and the fluxgate alone, so $\boldsymbol{\Psi}$ does not depend on the
estimated position---confounding (iii) can fail only if a $\boldsymbol{\Psi}$ column
happens to coincide with the map-gradient time series, never by construction.
Enlarging such a basis can slow the coefficient estimate through (ii) but adds no
structural coupling to position. This is the rule behind every estimator in
Section~\ref{sec:results}: keep the basis exogenous.
\end{remark}

The residual risk for an exogenous basis is exactly that coincidence of the flown
attitude history with the local map gradient---plausible, for instance, when turns
always occur at the same map features---and one expects (iii) to hold for
trajectories and maps in general position once the trajectory excites enough
position directions. We state this as an expectation, not a theorem: formalizing
``general position'' for real flight paths is beyond this paper, and
Section~\ref{sec:results} shows that scalar proxies for the intersection are
...
actionable consequence is the exogeneity rule (Remark~\ref{cor:exo}), which every
estimator in Section~\ref{sec:results} follows.
...
gradient. It explains two failure modes seen in
...
converge, and exogeneity (Remark~\ref{cor:exo}) is why the attitude-only TL
basis stays safe at any dimension. The map-information floor
