...
gradiometry~\cite{noriega2015}. The exception is the online NN-in-EKF line of work
...
online EKF+TL+NN baseline, and Section~\ref{sec:results} reports and
...
This collapse is observed empirically in Section~\ref{sec:baseline}: the fit
residual stays small while the position drifts to kilometres. It is why the scalar
magnetometer must be excluded from any compensation basis.
\end{remark}
...
the compensator silently absorbs the map signal.
\end{remark}

\section{The Online EKF+TL+NN Baseline}\label{sec:baseline}
The strongest causal competitor is the online NN-in-EKF lineage: the filter
augments the Pinson error state with the online TL coefficients
$\boldsymbol{\beta}$ and the weights of a small feed-forward network, all
propagated as EKF states and updated online against~\eqref{eq:meas}. The lineage
originates with Gnadt, whose MagNav.jl package provides its reference
implementation~\cite{gnadt2022}, and was recently extended
in~\cite{hager2026}. We baseline against the reference implementation, tuned by
us for a stable cold start; we do not claim to reproduce the specific extended
filter of~\cite{hager2026}.
...
scalar magnetometer creates an endogenous feature and collapses horizontal position
by the cancellation of Remark~\ref{rem:endo}. (iii)~We inflate the measurement
...
The tuned filter carries the permanent TL group and a single-hidden-layer
...
(Table~\ref{tab:1007}), consistent with the cold-start operating points published
for this lineage on this line~\cite{gnadt2022,hager2026}. All numbers compared in
...
to \num{32.6}/\SI{14.2}{\meter}. Our online EKF+TL+NN baseline reaches
\num{40.0}/\SI{17.5}{\meter} (Section~\ref{sec:baseline} details the
implementation and its relation to the published operating points). The window smoother beats
...
\midrule
EKF+TL+NN, online~\cite{gnadt2022,hager2026} & 40.0 & 17.5 \\
\bottomrule
\end{tabular}
\end{table}
...
baselines: the plain online-TL EKF, the online EKF+TL+NN of
