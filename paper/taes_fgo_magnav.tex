...
\title{Factor-Graph Aeromagnetic Compensation for Airborne Magnetic-Anomaly Navigation}
...
\markboth{LEE ET AL.}{FACTOR-GRAPH AEROMAGNETIC COMPENSATION FOR MAGNETIC-ANOMALY NAVIGATION}
\maketitle
...
online-TL EKF and a marginalized particle filter both diverge, and it beats a
reimplemented EKF+TL+NN, which the neural network keeps bounded, without using a
neural network. A simulation Monte-Carlo
confirms that the reported covariance is consistent and never over-confident. We
also give an exact linear condition for the joint observability of compensation and
navigation, from which an exogeneity design rule for the compensation features
follows.
\end{abstract}
...
magnetometer~\cite{canciani2016,canciani2017}. It is also inherently hard to deny.
...
but are in fact orthogonal. The first is \emph{temporal}: the coefficients are
...
than the calibrated case. The second is \emph{representational}: however well its
...
Most work on it attacks the two difficulties \emph{offline}: the TL coefficients by a
...
gradiometry~\cite{noriega2015}. The exception most relevant here is the online
EKF+TL+NN of~\cite{hager2026}, which carries an online-trained residual network
inside the filter to mop up the un-modeled remainder, and is, to our knowledge, the
strongest reported cold-start result on the benchmark. On top of the platform field,
the scalar optically-pumped magnetometers used for the cabin channels add their own
orientation-dependent errors---heading error and equatorial dead
zones~\cite{hager2026b,wang2020}. This paper takes on the \emph{temporal} difficulty
fully---online, joint estimation of a time-varying compensation---and the
physically structured part of the representational one, namely the scalar-sensor
errors; the unstructured, current-driven remainder that a network is designed to
absorb is left outside its scope.
...
cabin magnetometers and is, to our knowledge, the strongest reported cold-start
result on that benchmark. Nevertheless, the network requires a carefully
...
together with aircraft position from map matching, has not been posed in a factor
graph, and the observability of doing so has not been characterized.
...
information corrects early navigation states, (ii)~represents the compensation (and
the physical sensor errors of the scalar magnetometer) as first-class,
time-varying variables rather than a black box, and (iii)~comes with an explicit
condition for when position and compensation can be separated at all. This paper
supplies all three.
...
\begin{enumerate}
  \item \textbf{Factor-graph MagNav with joint online compensation.} We give, to our
        knowledge, the first formulation of magnetic-anomaly navigation as
        factor-graph (batch MAP) estimation, and carry the online Tolles--Lawson
        coefficients as time-varying variables of a fixed-lag sliding-window graph so
        that navigation and compensation are estimated jointly. The per-window
        relinearization makes the compensation adapt along the flight---the role a
        learned online model plays in the competing filter---while the graph keeps
        every quantity an explicit, physically interpretable state
        (Section~\ref{sec:method}). We are deliberate about what is claimed: for the
        linear--Gaussian error model the batch estimate equals the RTS smoother, so
        smoothing per se is not the contribution; the joint, windowed estimation of a
        time-varying compensation is.
  \item \textbf{Exact joint-observability condition.} We prove a necessary and
        sufficient linear-algebraic condition under which the aircraft position
        and the compensation coefficients are jointly observable from the
        scalar magnetometer, separating three distinct failure modes (an
        unexcited trajectory, a rank-deficient compensation basis, and the
        subtle one, a compensation subspace that is confounded with the anomaly
        gradient). From it we derive a concrete exogeneity design rule for
        choosing compensation features that a filter tuned by trial and error
        cannot see (Section~\ref{sec:observability}).
  \item \textbf{Physical sensor-error factors.} We add physically motivated
        optically-pumped-magnetometer error terms: a harmonic heading error carried
        as extra graph variables, and a dead-zone attenuation handled as a
        heteroscedastic reweighting of the scalar measurement. A controlled
        cumulative ablation then shows which of them are load-bearing. These are
        model terms that a position-only estimator cannot represent, and they let
        the graph absorb structured sensor error that would otherwise leak into the
        position estimate (Section~\ref{sec:method}, Table~\ref{tab:sensor}).
  \item \textbf{Cold-start capability, validated against a reproduced baseline.}
        ...
        this fairly we reimplement, rather than merely cite, the online EKF+TL+NN
        of~\cite{hager2026} and compare it, a weak online-TL EKF, and a marginalized
        ...
        network buys (it keeps the causal filter bounded where online TL alone and
        the particle filter diverge) from what it does not (on every full-length line
        the smoother is the most accurate of the four), a like-for-like evaluation the
        literature has lacked (Section~\ref{sec:results}).
\end{enumerate}
...
reimplemented EKF+TL+NN baseline, and Section~\ref{sec:results} reports and
...
online TL compensation, and optional sensor-error variables, solved over a
fixed-lag window.}
\label{fig:pipeline}
\end{figure*}
...
\subsection{Joint estimation problem}
The cold-start setting couples the two preceding models through a single scalar
stream: the same measurement $z_t$ must simultaneously fix position through the map
term $h(\mathbf{p}_t)$ and identify the compensation
$\mathbf{A}_t^{\top}\boldsymbol{\beta}_t$. We therefore state the estimation task as
one joint problem rather than a navigation problem preceded by a calibration step.

\begin{problem}[Joint compensation and navigation]\label{prob:joint}
Given the scalar measurements $z_{1:N}$ of~\eqref{eq:meas}, the fluxgate regressor
rows $\mathbf{A}_{1:N}$ of~\eqref{eq:tlbasis}, the anomaly map $h$, the INS
mechanization entering $\boldsymbol{\Phi}_t$, and the prior
$(\hat{\boldsymbol{\chi}}_0,\mathbf{P}_0)$, estimate the joint trajectory
$\boldsymbol{\chi}_{1:N}$ of navigation errors and compensation coefficients,
$\boldsymbol{\chi}_t=(\mathbf{x}_t,\boldsymbol{\beta}_t)$.
\end{problem}

We take the maximum-a-posteriori (MAP) estimate. By the Markov structure of the
dynamics~\eqref{eq:dyn}, the coefficient random walk, and the memoryless
measurement~\eqref{eq:meas}, the joint posterior factorizes as
\begin{equation}
p(\boldsymbol{\chi}_{1:N}\!\mid\! z_{1:N})\propto
p(\boldsymbol{\chi}_1)\!\prod_{t=1}^{N-1}\!p(\mathbf{x}_{t+1}\!\mid\!\mathbf{x}_t)\,
p(\boldsymbol{\beta}_{t+1}\!\mid\!\boldsymbol{\beta}_t)\!\prod_{t=1}^{N}\!
p(z_t\!\mid\!\boldsymbol{\chi}_t).
\label{eq:factorization}
\end{equation}
This factorization \emph{is} a factor graph~\cite{kschischang2001,dellaert2017}: a
bipartite graph whose variable nodes are the $\boldsymbol{\chi}_t$ and whose factor
nodes are the individual densities in~\eqref{eq:factorization}, each connected only
to the variables it involves (here, two parallel chains coupled by the measurement
factors; Fig.~\ref{fig:graph}). MAP estimation on such a graph is sparse nonlinear
...
flight scale (Section~\ref{sec:method}-E). Concretely, the MAP estimate minimizes
\begin{equation}
\begin{aligned}
...
the prior term anchors both blocks of $\boldsymbol{\chi}_1=(\mathbf{x}_1,
\boldsymbol{\beta}_1)$, at cold start with $\boldsymbol{\beta}$-block
$\mathbf{P}_0^{\beta}$ (Table~\ref{tab:params}).
...
Section~\ref{sec:method}-F attach to every measurement factor when enabled. Two
...
\emph{early} epochs are revised retroactively; and the robust kernel, unlike a
...
robust kernel, already stays bounded on every counted case
(Section~\ref{sec:results}-D), so the load-bearing mechanism is the windowed
...
EKF+TL+NN of~\cite{hager2026} devotes a neural network precisely to this remainder.
...
samples cannot pull the position estimate. The observability condition of
Section~\ref{sec:observability} is what keeps this well-posed: $S_t$ and
$\boldsymbol{\beta}_t$ may only absorb signal that is not collinear with the
map gradient $\mathbf{g}_t$, so genuine map information is not silently explained
away as interference. The trade it makes, a higher compensation residual than the
network in exchange for navigation robustness, is quantified with the cold-start
results in Section~\ref{sec:results}-C, and the account itself is checked against
the recorded platform-current channels in Section~\ref{sec:results}-D.
...
node $\boldsymbol{\theta}$ holds the optional sensor-error variables.}
\label{fig:graph}
\end{figure*}
...
data. Carrying the full covariance, not just the mean, keeps navigation
continuous across window boundaries. One approximation deserves note: the carried
prior is the smoothed state at the commit boundary, which has already seen
the overlap measurements, and window $k{+}1$ processes those $L_o$ epochs again, so
the overlap information is counted twice. An exactly consistent scheme would carry
the forward-filtered state or marginalize the overlap; we accept the bounded
optimism (the overlap is a fixed fraction, $L_o/L_w\!=\!0.3$, of each window) in
exchange for the simpler implementation, and note that the reported DRMS is a
comparison of means, unaffected by the covariance bookkeeping.
...
\SI{2}{\minute} window in Table~\ref{tab:1007} shows that shortening $L_w$ degrades
...
(typically $3$--$5$; Sec.~\ref{sec:results}-F). This
...
Lemma~\ref{lem:rts} does double duty. Practically, it licenses the fast
iterated-RTS implementation used for the joint TL problem, and we verify the QR and
RTS paths agree to within a metre on a real-data segment in
Section~\ref{sec:results}-A (that check covers the navigation-only chain).
Conceptually, it fixes the scope of the contribution: for a static, known
compensation the batch graph and a filter--smoother pass are equivalent, so nothing
here rests on ``smoothing beats filtering.'' What the graph adds beyond
Lemma~\ref{lem:rts} is exactly what the lemma's hypotheses exclude: the jointly
estimated, time-varying $\boldsymbol{\beta}$, the non-Gaussian robust kernel
of~\eqref{eq:huber}, the sensor-error variables of Section~\ref{sec:method}-F, and
the fixed-lag windowing that keeps latency bounded.
...
$\boldsymbol{\chi}$ with three physically motivated scalar-magnetometer error
variables, each appearing additively in the residual~\eqref{eq:resid}, plus one
measurement re-weighting:
\begin{enumerate}
\item OPM heading error, modeled as the cosine-harmonic expansion of the
optically-pumped (scalar-atomic) magnetometer in the sensor--field angle
$\psi_t=\angle(\hat{\mathbf{e}},\mathbf{B}_t)$,
$b^{\mathrm{hdg}}_t=\sum_{n\in\{1,2,4\}}c_n\cos(n\psi_t)$~\cite{hager2026b,wang2020},
with coefficients $\{c_n\}$ as graph variables; the $n{=}1$ term is the vector
light shift and $n{=}2$ the nonlinear Zeeman effect. Because $\psi_t$ is known
from attitude and IGRF, the term is linear in $\{c_n\}$ and, as in the
Tolles--Lawson case~\cite{wang2020}, enters the graph as an ordinary linear
regressor;
\item Hard-iron bias $b^{\mathrm{hi}}$, a constant offset variable;
\item Linear drift $b^{\mathrm{dr}}_t=\gamma_0+\gamma_1 t$, capturing slow
electronics drift;
\item Dead-zone weight: not a graph variable but a heteroscedastic
measurement information $R_t^{-1}=R^{-1}\max(|\cos\psi_t|,\epsilon)^2$
($\epsilon{=}0.05$) that follows the OPM signal-amplitude collapse near the
equatorial dead zone (field $\perp$ optical axis)~\cite{hager2026b} and
de-weights those samples (a de-weighting any estimator could apply; it is
included for completeness of the sensor model).
\end{enumerate}
...
position--compensation coupling which a full observability Gramian would mix with
the INS dynamics.
...
follows by taking $\delta\boldsymbol{\beta}'$ free and $\delta\mathbf{p}'=\mathbf{0}$.
...
Suppose every compensation feature is exogenous: $\mathbf{A}_t$ is a function of
aircraft attitude and the fluxgate alone, so $\boldsymbol{\Psi}$ does not depend on
the estimated position. Then two conditions are necessary for (iii):
the window must satisfy
$N\ge\mathrm{rank}(\mathbf{G})+\mathrm{rank}(\boldsymbol{\Psi})$, and no
$\boldsymbol{\Psi}$ column may be a linear combination of the map-gradient time
series. In particular, enlarging an exogenous basis can slow the coefficient
estimate through (ii) but creates no structural coupling to position.
\end{corollary}

For an exogenous basis, condition (iii) can fail only through coincidence of the
flown attitude history with the local map gradient---plausible, for instance, when
turns always occur at the same map features---and one expects it to hold for
trajectories and maps in general position once the dimension count above is met. We
state this as an expectation, not a theorem: formalizing ``general position'' for
real flight paths is beyond this paper, and Section~\ref{sec:results} shows that
scalar proxies for the intersection are unreliable in practice. The dependable rule
is the structural one: keep the basis exogenous.

\begin{remark}[Endogenous features cancel the position channel]\label{rem:endo}
An endogenous feature breaks the model's premise rather than condition (iii). If a
compensation feature is itself a function of position---the measured scalar $z_t$
is the canonical example---then $\boldsymbol{\Psi}$ is no longer a known
parameter, and the linearization must charge the feature's own position sensitivity
to the state. For the scalar reading that sensitivity is exactly the map gradient,
$\partial z_t/\partial\mathbf{p}=\mathbf{g}_t$, so with fitted coefficient
$\beta_z$ on that feature the effective position row of the measurement becomes
$(1-\beta_z)\,\mathbf{g}_t$: as the regression drives $\beta_z\!\to\!1$, the fit
residual shrinks while the measurement loses its position information entirely.
This is the collapse observed empirically in Section~\ref{sec:baseline}, where the
residual stays small as the position drifts to kilometres, and it is why the scalar
magnetometer must be excluded from any compensation basis, learned or linear.
\end{remark}

\begin{remark}[TL rank deficiency as gauge freedom]\label{rem:gauge}
...
condition (i) fails: position is unobservable regardless of the
compensation, a map-information floor that bounds every estimator, and that we
observe on the shortest line in Section~\ref{sec:results}. Second, an endogenous
feature cancels the position channel by the mechanism of Remark~\ref{rem:endo}, so
the compensator silently absorbs the map signal; excluding such features
(Section~\ref{sec:baseline}) is necessary for the baseline to converge.
\end{remark}

\section{Reimplemented Online EKF+TL+NN Baseline}\label{sec:baseline}
To compare against~\cite{hager2026} on equal footing we reimplement, rather than
merely cite, its online EKF+TL+NN cold start. The filter augments the Pinson error
state with the online TL coefficients $\boldsymbol{\beta}$ and the weights of a
small feed-forward network, all propagated as EKF states and updated online
against~\eqref{eq:meas}. Since neither the network architecture nor the training
design of~\cite{hager2026} is released, ours reproduces the published design
principles rather than the exact filter.

A naive port diverged, and three fixes, each addressing an independently diagnosed
failure, were needed. (i)~We remove the large constant offset from the regression
target and initialize the network output bias to it, because the raw offset
($-1724\,\mathrm{nT}$ on Mag~4, $+310\,\mathrm{nT}$ on Mag~5) otherwise drives the
network bias to $\sim\!5\times10^{4}$\,nT and the filter to \texttt{NaN}. (ii)~We
restrict the network inputs to the attitude-derived TL basis, because feeding the
scalar magnetometer creates the endogenous feature of the Remark above and collapses
horizontal position (Corollary~\ref{cor:exo}). (iii)~We inflate the measurement
variance and the coefficient process noise through the warm-up, when the
compensation is still grossly wrong, then relax them.

The reimplemented filter carries the permanent TL group and a single-hidden-layer
network of eight units on that group (\num{41} network-weight states), with a FOGM
$(\sigma,\tau)=(\SI{3}{\nano\tesla},\SI{180}{\second})$ and post-warm-up measurement
variance $12^2\,\mathrm{nT}^2$. It reaches \num{40.0}/\SI{17.5}{\meter} on Mag~4/5
(Table~\ref{tab:1007}): inside the reported \SIrange{37}{58}{\meter} band on Mag~4,
and \SIrange{2.5}{3.5}{\meter} above the reported \SIrange{14}{15}{\meter} on Mag~5.
This is a functioning but not exact reimplementation, which we use as the baseline.
...
a calibration-maneuver flight, so we set its single short line aside
(Section~\ref{sec:results}-D), and the map-building flights (Flt1004--1005) are not
...
observability by exciting the regressor (Section~\ref{sec:observability}), so
...
(\SI{37.0}{}$\rightarrow$\SI{32.6}{\meter} on Mag~4), so the reader can attribute
...
EKF's \SI{28.3}{\meter} to \SI{14.0}{\meter}, against \SI{114.6}{\meter}
...
to \num{32.6}/\SI{14.2}{\meter}. Our reimplemented online EKF+TL+NN reaches
\num{40.0}/\SI{17.5}{\meter} (Section~\ref{sec:baseline} details how close this is
to the reported operating point), while the reported TL-only EKF is weaker still on
Mag~4. The window smoother beats the reimplemented NN-augmented filter on both
sensors without any neural network, using only the linear TL basis carried
as graph variables; against the values reported in~\cite{hager2026} it is
\SI{4.4}{\meter} better on Mag~4 and \SI{0.2}{\meter} behind on Mag~5. This
comparison against the published values is on the single line 1007.06, the only
line~\cite{hager2026} reports; our reimplemented EKF+TL+NN carries the baseline to
the other lines in the breadth study (Table~\ref{tab:breadth}). We attribute the
...
Fig.~\ref{fig:winlen} plots DRMS at the three lengths evaluated on this line: the
\SI{2}{\minute} window under-determines the coefficients, the full-line static
batch cannot track their drift, and the \SI{5}{\minute} window is the best of the
three. The tradeoff mirrors the expressiveness discussion of
Section~\ref{sec:observability} and makes $L_w$ a physically interpretable knob
rather than a free hyperparameter, though a denser sweep (and other lines, where
the balance can shift toward longer windows) is left for the revision.
...
\midrule
EKF+TL+NN, reimplemented~\cite{hager2026} & 40.0 & 17.5 \\
EKF+TL+NN, reported~\cite{hager2026} & 37 & 14 \\
EKF+TL-only, reported~\cite{hager2026} & 58 & 15 \\
\bottomrule
\end{tabular}
\end{table}
...
reimplemented and reported EKF+TL+NN (right).}
\label{fig:coldstart}
\end{figure}
...
survey traverses (Flt1003), grouped by purpose, against three
baselines: the plain online-TL EKF, the reimplemented EKF+TL+NN of
Section~\ref{sec:baseline} (the strong causal method that a fair comparison
demands), and a marginalized (Rao-Blackwellized) particle filter extended to carry
the TL coefficients in its conditionally-linear-Gaussian block (MPF+TL), a genuine
non-Gaussian Bayesian estimator rather than a linearization.
...
to tens of kilometres on two lines and runs off the map on a third. The particle
filter is no more robust---it diverges past \SI{10}{\kilo\meter} on every counted
case, because per-particle map-matching still cannot re-linearize the early flight
once the coefficients drift. The neural network is what fixes this: the EKF+TL+NN
stays finite on all eight cases, confirming that its residual network is what buys
cold-start robustness for the causal filter. The window smoother matches that
robustness without any neural network (it is bounded on all eight) and improves on
accuracy: across the four full-length lines (\SIrange{63}{87}{\minute}, eight
mag-cases) it is the best of the four on all eight, beating the NN-augmented filter
by \SIrange{4}{91}{\meter} (Fig.~\ref{fig:breadth}).
...
on this benchmark. One caveat on provenance: the EKF+TL+NN is our reimplementation,
matching the reported 1007.06 operating point to within a few metres
(Section~\ref{sec:baseline}), but the original's per-line numbers on the other
...
per-line DRMS varies by order \SI{1}{}--\SI{2}{\meter} between runs; the values here
are those of the committed continuous-integration run and do not affect the
best-of-four ordering.
...
map; bold = best of four. MPF+TL is a Rao-Blackwellized particle filter carrying the
TL coefficients; it diverges on every counted case. Calibration line 1006.08 set
aside (see text).}
\label{tab:breadth}
\centering
\setlength{\tabcolsep}{3pt}
\begin{tabular}{llccc rrrr}
\toprule
 & & & alt & & EKF & EKF+ & MPF & FGO \\
Line & Purp. & Map & [km] & Mag & online & TL+NN & +TL & win \\
\midrule
1007.06 & FF & R & 0.4 & 4 & 46.7 & 48.9 & div. & \textbf{32.7} \\
1007.06 & FF & R & 0.4 & 5 & 17.8 & 18.3 & div. & \textbf{13.8} \\
1007.02 & FF & E & 0.8 & 4 & div. & 130.0 & div. & \textbf{38.6} \\
1007.02 & FF & E & 0.8 & 5 & 31.6 & 30.4 & div. & \textbf{14.5} \\
1003.02 & SV & E & 0.4 & 4 & div. & 99.1 & div. & \textbf{42.6} \\
1003.02 & SV & E & 0.4 & 5 & 28.1 & 29.5 & div. & \textbf{21.7} \\
1003.08 & SV & R & 0.4 & 4 & err. & 46.6 & div. & \textbf{26.1} \\
1003.08 & SV & R & 0.4 & 5 & 21.1 & 20.7 & div. & \textbf{12.4} \\
\midrule
...
filter diverges on every counted case and is omitted.}
\label{fig:breadth}
\end{figure}
...
we tried, including the marginalized particle filter, which diverges on all eight
counted cases despite being a full Bayesian estimator: bounded cold-start
compensation without a network requires the batch/window re-smoothing, not merely a
non-Gaussian representation. An ablation within our own estimator locates the
mechanism more precisely: running the same window with the robust kernel disabled
still stays bounded on all eight counted cases (e.g.\ \SI{66.3}{\meter} on
1003.02/Mag~4 where the causal filter reads \SI{41626}{\meter}, refined to
\SI{42.6}{\meter} by Huber), so the robustness comes from the windowed
...
be read with care, because our EKF+TL+NN is a reimplementation and part of its
...
The observability condition of Section~\ref{sec:observability} accounts for the
pattern. The endogeneity cancellation (Remark~\ref{rem:endo}) is why the scalar
magnetometer had to be excluded from the network inputs for the baseline to
converge, and exogeneity (Corollary~\ref{cor:exo}) is why the attitude-only TL
basis stays safe at any dimension. The map-information floor of the
Remark is why the short calibration line 1006.08 is hard for every method, since
...
To exercise the sensor-error factors of Section~\ref{sec:method} we inject the
optically-pumped-magnetometer error model of~\cite{hager2026b,wang2020} into a
simulated flight (free-inertial reference \SI{31.1}{\meter}): a cosine-harmonic
heading error at orders $\{1,2,4\}$ (\SI{7.6}{\nano\tesla} RMS), a fluxgate
hard-iron bias, a linear drift, and the equatorial-dead-zone noise amplification
$1/\max(|\cos\psi|,\epsilon)$. We then enable the corresponding graph variables
cumulatively; Table~\ref{tab:sensor} reports the result. With no sensor-error
states the estimate is worse than free inertial (\SI{42.9}{\meter}): the
unmodeled error corrupts the map aiding, exactly the mechanism of
Proposition~\ref{prop:obs}. Enabling all four factors reduces the error to
\SI{6.2}{\meter}, an \SI{86}{\percent} reduction, monotone in this order. Because
the factors are enabled cumulatively in a single order, Table~\ref{tab:sensor}
attributes marginal gains only for that order: the largest single-step drop comes
from the heading harmonics ($42.9\!\rightarrow\!18.9$\,m), followed by the
dead-zone weighting ($\rightarrow\!15.5$\,m, now load-bearing because the injected
dead-zone noise is heteroscedastic), the hard-iron bias
($\rightarrow\!8.1$\,m), and the linear drift, whose rate the estimator recovers
cleanly ($\hat{\gamma}_1\!=\!0.018$ versus a true $0.020$). The heading
coefficients themselves are poorly identified ($\hat{c}_1\!=\!3.8$ versus a true
$10$): level flight sweeps the sensor--field angle by only about
\SI{12}{\degree}, so the harmonic regressor is barely excited, an observability
property of the trajectory, consistent with the Remark of
Section~\ref{sec:observability}, that a maneuvering flight would relieve, and the
gain to navigation comes from the fitted combination over the observed angle
range, not from the individual coefficients. The Huber kernel gives similar values
(within \SI{2}{\meter}; it helps most at the dead-zone step). Two caveats bound
this study: it is simulation-only, and the injected errors share the estimator's
functional form, so it demonstrates that the graph formulation can carry and
recover such states, not that these models capture real sensor behavior.

\begin{table}[t]
\caption{Sensor-error factors (simulation), enabled cumulatively; DRMS [m].
Free-inertial \SI{31.1}{\meter}; non-robust solver (Huber within \SI{2}{\meter}).}
\label{tab:sensor}
\centering
\begin{tabular}{lr}
\toprule
...
$+$ hard-iron bias & 8.1 \\
\textbf{$+$ linear drift (full)} & \textbf{6.2} \\
\bottomrule
\end{tabular}
\end{table}
...
conservative uncertainty that bounds the true error with margin. The particle filter
is the opposite. Its ANEES is \num{59.8}, more than an order of magnitude above
\num{2}, so its reported position covariance is severely over-confident---the
particle-depletion signature, in which the surviving particles after resampling on a
high-resolution map cluster far more tightly than the true error and the empirical
covariance collapses. The two linearized estimators are credible; the particle
filter, though DRMS-competitive with the EKF, is not. Fig.~\ref{fig:consistency}
shows the EKF and factor-graph NEES staying inside the \SI{95}{\percent} $\chi^2_2$
band throughout a representative run while the particle filter runs far above it, and
...
and batch FGO) with the \SI{95}{\percent} $\chi^2_2$ band; the EKF and FGO stay
inside it while the particle filter sits far above (ideal \num{2}, dotted).}
...
(representative simulated FGO run).}
\label{fig:sigma}
\end{figure}
...
These same properties make the estimator compatible with real-time operation. Its
committed output lags real time by the overlap $L_o$, here \SI{90}{\second}, a
latency that suits the map-aiding role, in which the INS supplies the high-rate
solution and the smoother corrects it periodically, and that the designer trades
against accuracy through $L_o$. The factor interface is modular, so the
compensation, sensor-error, and map factors can be added or removed without touching
the solver, and one implementation then covers the calibrated case with frozen
coefficients, the cold-start case with adaptive coefficients, and the
augmented-sensor case. Integrating it with an operational system amounts to feeding
...
navigation states, the mechanism a one-pass causal filter lacks; physically
modeled scalar-magnetometer error terms (heading harmonics, hard-iron bias, drift,
and dead-zone reweighting) attach to the same graph as ordinary variables; and a
necessary-and-sufficient joint-observability condition, with its exogeneity rule
and endogeneity cancellation, states when position and compensation can be
separated at all. On real SGL~2020 data the estimator stays bounded from a cold
start on all eight full-length cabin-magnetometer cases, where both the plain
online-TL EKF and a marginalized particle filter diverge, and it is more accurate
on every such case than a reimplemented online EKF+TL+NN, with no neural network
and no calibration flight. A simulation Monte-Carlo shows its reported covariance
is consistent and conservative where the particle filter's is over-confident, and
a cumulative ablation shows the physical sensor-error factors recover most of an
injected sensor-error budget.
...
map-matcher~\cite{bergman1999,nordlund2009}. The sensor-error factors are
validated against injected, model-matched errors in simulation, not against a
physical optically-pumped magnetometer. And none of the SGL~2020 lines is a long,
...
full SGL line set and a grid or point-mass baseline; demonstrating the
sensor-error factors on flight data with genuine optically-pumped-magnetometer
heading error, with maneuvers wide enough to make the harmonic coefficients
cleanly identifiable; testing a low-excitation operational cruise; and
sharpening the geometric observability condition into a quantitative
identifiability screen. None of these qualifications affects the central and
