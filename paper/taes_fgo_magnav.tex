...
missions, the smoother is the best of three
cold-start estimators on every full-length line (\num{8} of \num{8} cases): it stays bounded where a plain
online-TL EKF diverges, and it beats a reimplemented EKF+TL+NN, which the neural
network keeps bounded, without using a neural network. A simulation Monte-Carlo
...
\begin{enumerate}
  ...
  \item \textbf{Reimplemented baseline and multi-line breadth study.} We
        ...
        \cite{hager2026} and place it head-to-head with the proposed smoother and a
        weak online-TL EKF on eight cold-start cases spanning four long lines, two
        ...
        diverges) and what it does not (on every full-length navigation line the
        NN-free smoother is the most accurate of the three), giving a like-for-like
        comparison that the literature has lacked (Section~\ref{sec:results}).
...
survey traverses (Flt1003), grouped by purpose, against two causal
baselines: the plain online-TL EKF, and the reimplemented EKF+TL+NN of
Section~\ref{sec:baseline}, the strong causal method that a fair comparison demands.

The two baselines behave very differently, and the contrast is the point. The
...
network fixes exactly this: the EKF+TL+NN stays finite on all eight cases,
...
full-length lines (\SIrange{63}{87}{\minute}, eight mag-cases) it is the best of the
three on all eight, beating the NN-augmented filter by
\SIrange{4}{76}{\meter} (Fig.~\ref{fig:breadth}).
...
warm-up leaves a \SI{4}{\minute} window over which none of the three DRMS values is
...
accuracy conclusion from it: in particular the NN's \SI{34.6}{\meter} on its Mag~5,
against \SIrange{117}{122}{\meter} for the TL-only methods, is not evidence of an NN
...
EKF (1003.02: $33.8$ vs \SI{28.1}{\meter}) when its network has not yet learned
...
map; bold = best of three. Calibration line 1006.08 set aside (see text).}
\label{tab:breadth}
\centering
\setlength{\tabcolsep}{3pt}
...
\midrule
1007.06 & FF & R & 0.4 & 4 & 46.7 & 42.2 & \textbf{32.7} \\
1007.06 & FF & R & 0.4 & 5 & 17.8 & 17.8 & \textbf{13.8} \\
1007.02 & FF & E & 0.8 & 4 & div. & 114.8 & \textbf{38.6} \\
1007.02 & FF & E & 0.8 & 5 & 31.6 & 29.3 & \textbf{14.5} \\
1003.02 & SV & E & 0.4 & 4 & div. & 99.1 & \textbf{42.6} \\
1003.02 & SV & E & 0.4 & 5 & 28.1 & 33.8 & \textbf{21.7} \\
1003.08 & SV & R & 0.4 & 4 & err. & 46.4 & \textbf{26.1} \\
1003.08 & SV & R & 0.4 & 5 & 21.1 & 18.8 & \textbf{12.4} \\
\midrule
...
(gray), EKF+TL+NN (red), and the NN-free FGO window (blue).}
\label{fig:breadth}
\end{figure}
...
state-of-the-art filter. What the window adds is that it reaches the same bounded
...
best of the three on all eight full-length cases. This margin is real, but it should
...
scalar-measurement realization, then run the causal EKF and the batch factor graph
on the same draw. The clean compensated measurement isolates the navigation
covariance from the cold-start compensation transient.

Averaged over the \num{30} seeds, after a \SI{30}{\second} warm-up, the horizontal
DRMS is \SI{7.4}{\meter} for the factor graph and \SI{9.3}{\meter} for the EKF, with
\SI{95}{\percent} confidence half-widths of \SI{1.6}{} and \SI{1.3}{\meter}. On this
clean sensor both estimators sit near the map-limited floor, so the intervals
overlap and the gap is modest; the large accuracy separation is the cold-start
regime of Sections~\ref{sec:results}-C and \ref{sec:results}-D, not this one. The
purpose of this study is consistency.
...
factor graph and \num{1.11} for the EKF, both below the ideal \num{2}. A value below
\num{2} means the reported uncertainty is conservative and bounds the true error with
margin, whereas a value above \num{2} would signal an over-confident covariance.
Neither estimator is over-confident, and the smoother covariance is the better
calibrated of the two, closer to the ideal. Fig.~\ref{fig:consistency} shows the
NEES staying inside the \SI{95}{\percent} $\chi^2_2$ band throughout a representative
run, and Fig.~\ref{fig:sigma} shows the North and East position error contained
within the estimator $\pm2\sigma$ envelope. Two caveats bound the reading. The NEES
uses the per-axis (diagonal) covariance, since the cross term is not exposed by the
evaluated filter output, and the study is a matched-model simulation, so it
certifies internal consistency rather than robustness to model mismatch. The
conservative bias indicates that the process noise could be tightened, which is the
safe direction in which to err.

\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{figs/fig_consistency.pdf}
\caption{Monte-Carlo position NEES versus time (EKF and batch FGO) with the
\SI{95}{\percent} $\chi^2_2$ band; both stay inside it (ideal \num{2}, dotted).}
...
covers a causal online-TL EKF and a reimplemented online EKF+TL+NN, but not a
like-for-like grid or particle-filter map-matching
baseline~\cite{bergman1999,nordlund2009}, which would sharpen the claim about
estimator structure. The sensor-error factors are validated in simulation only, and
...
with no neural network is the best of three cold-start estimators on all eight
full-length cases, bounded where the plain causal filter diverges and more accurate
than an NN-augmented causal filter that does use one.
