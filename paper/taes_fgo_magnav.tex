...
smoother that reaches the real-time limit at \SI{14}{\milli\second} per epoch. The
lag is the sole design variable, trading the future data that corrects a committed
...
decompose into a warm-prior half and a map-observability half. Across cold-start
lines from two flights and two maps, the smoother is the most accurate of four
estimators on every full-length case (\num{8} of \num{8}): it stays bounded where an
online-TL EKF diverges and a marginalized particle filter is erratic, and it beats
a tuned EKF+TL+NN baseline with no learned component. A simulation Monte-Carlo
confirms the reported covariance is consistent, and an independent GTSAM
reimplementation sharing no code reproduces the bounded result.
\end{abstract}
...
real-time limit. The lag is the single design variable along this spectrum, trading
observability---how much future data corrects a committed state---against latency and
...
\begin{enumerate}
  ...
  \item \textbf{A MAP estimation spectrum.} We solve that one MAP problem across a
        ...
        iSAM2. The lag is a single design variable trading observability against
        latency and computation, so the batch fixes the offline accuracy bound and
        the incremental smoother runs in real time (\SI{14}{\milli\second} per
        epoch). This places recursive filtering and batch smoothing as two limits of
  ...
  \item \textbf{Position--compensation separability and the cold-start transient.}
        ...
        position error can be told apart from a change in the compensation, separating
        ...
        experiments decompose: a warm compensation prior halves it, and the remaining
  ...
  \item \textbf{Flight-data validation.} On uncompensated cabin magnetometers across
        ...
        learned component. A simulation Monte-Carlo confirms the reported covariance
...
use; and the fixed-lag sliding window sits between, committing a state once it has
seen a bounded amount of future data. The lag is thus the single design variable that
...
(typically $3$--$5$; Sec.~\ref{sec:results}-G). This
...
start on line 1007.06, holds a horizontal DRMS near \SI{82}{\meter} through the first
several minutes before locking on (Section~\ref{sec:results}-G). Seeding the
compensation with a warm Tolles--Lawson prior, tightening its initial standard
deviation from \num{1.0} to \num{0.1}, removes only about half of this transient, to
...
warm compensation prior roughly halves the penalty, but both runs lock on only at
...
Levenberg--Marquardt and iSAM2 solvers and sharing no code with the in-house Julia
...
On the full \SI{87}{\minute} cold-start line the two agree closely: the GTSAM window
returns \SI{12.4}{\meter} horizontal DRMS against the Julia smoother's
\SI{13.8}{\meter}, and on the first ten-minute segment \SI{17.5}{} against
\SI{21.4}{\meter}; the free-inertial drift matches at \SI{317.5}{} against
...
interpolation, and a slightly different window handoff. Both are single runs, so this
is a statement about implementation independence rather than a statistical comparison;
its value is that the bounded, sub-\SI{15}{\meter} cold-start result survives a
complete change of language, library, and solver; Fig.~\ref{fig:crossimpl} overlays
the GTSAM error trace on the segment.
...
The per-epoch incremental smoother of Section~\ref{sec:incremental} realizes this
budget at the finest stride and lets us measure it directly. Running the iSAM2
fixed-lag smoother on line 1007.06 with a \SI{30}{\second} lag, one update per
\SI{0.1}{\second} sample, takes \SI{14}{\milli\second} per epoch, about seven times
faster than the \SI{10}{\hertz} sample rate and comfortably real-time; the figure
includes the Python measurement-factor callbacks and is therefore a conservative upper
bound, since native factors would be faster. A causal, zero-latency estimate exists at
every step. After it converges, the \SI{30}{\second}-lag realtime DRMS is
\SI{16.5}{\meter} from a cold Tolles--Lawson start and \SI{15.1}{\meter} with a warm
prior, close to the \SI{14.8}{\meter} of the \SI{300}{\second}-lag smoothed estimate
and far below the free-inertial drift (Fig.~\ref{fig:realtime}).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_realtime.pdf}
\caption{Causal per-epoch estimates on the ten-minute segment: the
\SI{30}{\second}-lag realtime error locks onto the map feature near four and a half
minutes and settles close to the \SI{300}{\second}-lag smoothed estimate, far below
free-inertial drift.}
\label{fig:realtime}
\end{figure}

The lag behaves as Section~\ref{sec:method} predicts. Lengthening it improves the
estimate, to \SI{14.8}{\meter} smoothed at a \SI{300}{\second} lag, but at a cost our
unoptimized Python callbacks cannot yet meet in real time at that lag, so a long lag is
presently an offline setting in this implementation; native factors would move it
online. Shortening the lag to \SI{30}{\second} restores real time at the price of the
cold-start transient analyzed in Section~\ref{sec:observability}: a warm compensation
prior halves that transient, and lag scheduling removes the rest by running a long lag
through the early, weakly observable minutes and contracting to \SI{30}{\second} once
the compensation has converged.
...
runs in real time at \SI{14}{\milli\second} per epoch. On real SGL~2020 data the
estimator stays bounded from a cold start on all eight full-length
...
tuned online EKF+TL+NN baseline, with no neural network and no calibration flight. A
simulation Monte-Carlo shows its reported covariance is consistent and conservative
...
statistical claim; distributional statements come from the simulation Monte-Carlo
...
folded into the online estimator. In its present Python form the per-epoch smoother
meets real time only at a short lag; the long lag that removes the cold-start
transient still runs offline, a limit we expect native factors to lift. And none of
