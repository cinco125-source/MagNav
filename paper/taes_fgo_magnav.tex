...
\begin{enumerate}
  ...
  \item \textbf{Exact joint-observability condition.} We prove a necessary and
        ...
        one, a compensation subspace confounded with the anomaly gradient
...
We run this reference filter on its standard attitude-derived TL features. The one
substantive adaptation a cold start forces is initialization, since there is no
calibration flight to warm-start from, and a naive port that skips it diverges: the
raw uncompensated offset ($-1724\,\mathrm{nT}$ on Mag~4, $+310\,\mathrm{nT}$ on
Mag~5) drives the network output bias to $\sim\!5\times10^{4}\,\mathrm{nT}$ and the
filter to \texttt{NaN} within seconds. We therefore initialize the network output
normalization data-drivenly from the first five minutes of onboard interference, and
set the NN-weight covariance by hand as an explicit adaptation-rate knob rather than
through the package's training-data warm start.

We keep the uncompensated scalar out of the feature set---the standard choice here,
and one the observability analysis makes mandatory. A position-dependent input has
sensitivity $\partial z_t/\partial\mathbf{p}=\mathbf{g}_t$, so a fitted coefficient
$\beta_z$ turns the effective position row into $(1-\beta_z)\mathbf{g}_t$; as
$\beta_z\!\to\!1$ the network quietly absorbs the map signal while the residual stays
small and horizontal position drifts to kilometres---condition (iii) of
Proposition~\ref{prop:obs} failing by construction. The recent augmentation of the
feature set with that scalar~\cite{hager2026} avoids this only by freezing the
network after its transient; ours keeps learning, where the endogenous input would
be fatal.
...
$(\sigma,\tau)=(\SI{3}{\nano\tesla},\SI{180}{\second})$ and measurement
...
necessary-and-sufficient joint-observability condition states when position and
compensation can be separated at all. On real SGL~2020 data the estimator stays bounded from a cold
...
position-independent basis the observability condition requires;
