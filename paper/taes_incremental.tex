...
a tactical-grade unit~\cite{hager2025lie}. That gain is on the propagation
side, and it is complementary to the question studied here: the study is in
simulation and assumes every magnetic source but the anomaly field to be
perfectly corrected beforehand, so the aircraft field never enters, and the
map term itself gains nothing from the group structure, its Jacobian still
being a finite difference of a map that is by construction not invariant to
position. Its reported divergence on low-information maps is the same
map-information floor we make explicit in Section~\ref{sec:obs}.
...
\begin{enumerate}
  ...
  \item \textbf{A separability condition and a flight-dependent lag metric.}
        ...
        is hardest, and it identifies where the estimate stops being limited
        by separability at all.
...
purchase of smoothing accuracy with latency. The ablation finds no delicate modelling
