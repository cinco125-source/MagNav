...
\caption{Concept of the study. An aircraft with an uncalibrated magnetic
signature flies over the anomaly map with no satellite aiding; the one scalar
reading mixes the map value with the aircraft field, coupling position and
compensation. A joint factor graph, solved incrementally at a fixed lag,
estimates both and emits a causal answer now and a committed answer revised by
the lag.}
\label{fig:concept}
\end{figure*}
...
\caption{Line 1007.06 Mag~4: the uncompensated aircraft field against the map
signal along the same track. The quantity to be removed exceeds the full range
of the quantity to be matched.}
\label{fig:signal}
\end{figure}
...
\caption{The graph the smoother maintains, exactly as implemented. Circles are
the joint states at the \SI{1}{\hertz} cadence; binary factors carry the
transition and process noise compounded over the $K$ intervening samples; a
robust map factor hangs from every state; everything older than the lag is
summarized by the marginal prior on the oldest retained state. The causal
output is read at the newest state, the committed output where states leave
the lag.}
\label{fig:fgbasic}
\end{figure}
...
\caption{Factor graph of the joint problem. The inertial error chain (top) and
the compensation chain (bottom) evolve under their own process factors and are
coupled at every epoch by the scalar map factor, which is the only place the
two unknowns meet, and the reason they must be estimated together.}
\label{fig:graph}
\end{figure*}
...
\begin{table}[t]
\caption{Flight lines. FF = free flight, SV = survey; maps R = Renfrew, E = Eastern; INS = free-inertial drift [m].}
\label{tab:lines}
\centering
...
\begin{table}[t]
\caption{Estimator settings, common to every line and both magnetometers.}
\label{tab:params}
\centering
\footnotesize
...
\caption{Line 1007.06 Mag~4. (a)~Flown line on the map; at this scale every
bounded method overlays the truth. (b)~The same paths in the north/east error
plane about the true position, after the warm-up: the committed estimate stays
tightest around the origin while the INS spirals out.}
\label{fig:track}
\end{figure}
...
\caption{What the compensation chain learned (Mag~4): the estimated $\mathbf{A}^{\top}\boldsymbol{\beta}+S$ overlays the interference reference; the residual is tens of nanotesla.}
\label{fig:comp}
\end{figure}


\begin{table}[t]
\caption{Line 1007.06 cold start: horizontal DRMS [m] after a \SI{600}{\second} warm-up. The causal output uses no look-ahead; the committed output lags by \SI{300}{\second}.}
\label{tab:1007}
\centering
\begin{tabular}{lrrc}
\toprule
...
\caption{North and east error histories of the compared methods on line 1007.06
Mag~4. The EKF+TL+NN trace is a fresh run of the seed-sensitive filter
(\SI{40.0}{\meter} on this run, against the fixed reference run of
Table~\ref{tab:1007}).}
\label{fig:latlon}
\end{figure}
...
\caption{Line 1007.06 Mag~4, cold start: horizontal error of the causal and the
committed estimates against free-inertial drift. The committed trace is the
causal trace revised by \SI{300}{\second} of look-ahead before each state is
frozen.}
\label{fig:errhist}
\end{figure}
...
\caption{Breadth comparison of Table~\ref{tab:breadth} on a log axis.}
\label{fig:breadthbar}
\end{figure}


\begin{table}[t]
\caption{Breadth: DRMS [m] after a \SI{600}{\second} warm-up. ``div.''\ $>\SI{10}{\kilo\meter}$, ``err.''\ leaves the map; bold = best causal entry.}
\label{tab:breadth}
\centering
...
\midrule
\multicolumn{5}{l}{best causal} & \textbf{6 / 8} & \\
...
nothing re-tuned (Fig.~\ref{fig:breadthbar}). The committed estimate is the most accurate entry on all
...
\caption{Committed DRMS against lag (Table~\ref{tab:lag}); open markers show the lag-independent causal error.}
\label{fig:lagsweep}
\end{figure}


\begin{table}[t]
\caption{Lag sweep: committed DRMS [m], causal at $L=\SI{300}{\second}$. The causal output varies by under a metre across the sweep.}
\label{tab:lag}
\centering
\setlength{\tabcolsep}{4pt}
...
\begin{table}[t]
\caption{Ablation on line 1007.06, one change per row: committed (causal) DRMS [m].}
\label{tab:ablation}
\centering
\setlength{\tabcolsep}{4pt}
...
measurement~\eqref{eq:measlin}, couples this block to the map.
