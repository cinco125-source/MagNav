...
\caption{Factor graph of the joint problem, as maintained by the fixed-lag
smoother. The inertial chain and the compensation chain evolve under their own
process factors and are coupled at every epoch by the robust map factor, which
reads the anomaly map and is the only place the two unknowns meet. States
older than the lag are summarized by the marginal prior; the causal output is
read at the newest state and the committed output where states leave the
window.}
\label{fig:graph}
\end{figure*}
...
\caption{Line 1007.06 Mag~4. (a)~Flown line on the map, with the zoom window
marked. (b)~Zoom, in latitude/longitude, centred where the post-warm-up EKF
error is largest: the estimated paths separate from the truth, with the
committed estimate hugging it and the INS displaced to the east.}
\label{fig:track}
\end{figure}
