...
estimates both and emits a causal answer now and a smoothed answer revised by
the lag.}
\label{fig:concept}
\end{figure*}
...
read at the newest state and the smoothed output where states leave the
window.}
\label{fig:graph}
\end{figure*}
...
smoothed estimate hugging it and the INS displaced to the east.}
\label{fig:track}
\end{figure}
...
\begin{table}[t]
\caption{Line 1007.06 cold start: horizontal DRMS [m] after a \SI{600}{\second} warm-up. The causal output uses no look-ahead; the smoothed output lags by \SI{300}{\second}.}
\label{tab:1007}
\centering
\begin{tabular}{lrrc}
\toprule
...
smoothed estimates against free-inertial drift. The smoothed trace is the
...
\begin{table}[t]
\caption{Breadth: DRMS [m] after a \SI{600}{\second} warm-up. ``div.''\ $>\SI{10}{\kilo\meter}$, ``err.''\ leaves the map; bold = best overall.}
\label{tab:breadth}
\centering
...
\caption{Smoothed DRMS against lag (Table~\ref{tab:lag}); open markers show the lag-independent causal error.}
\label{fig:lagsweep}
\end{figure}


\begin{table}[t]
\caption{Lag sweep: smoothed DRMS [m], causal at $L=\SI{300}{\second}$. The causal output varies by under a metre across the sweep.}
\label{tab:lag}
\centering
\setlength{\tabcolsep}{4pt}
