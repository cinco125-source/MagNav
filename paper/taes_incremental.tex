...
\end{itemize}
Both are components of one optimization. Writing
...
\caption{Separability metric $\mu(L)^{-1/2}$ of Eq.~\eqref{eq:mu} (dashed,
median and interquartile range over sliding windows) against the measured
smoothed DRMS (solid, Mag~4), per line.}
\label{fig:obsmetric}
\end{figure}
...
$\mu(L)^{-1/2}$ in sliding windows along each line at the \SI{1}{\hertz} kept
cadence. At \SI{30}{\second} the weakest-direction bound is
\SIrange{184}{1585}{\meter}, no useful window; it falls steeply with window
length and by \SI{300}{\second} reaches \SIrange{6.7}{19.3}{\meter}, at or
below the measured smoothed error, so past the knee separability is no longer
the binding constraint and the residual error is set by map error, inertial
noise, and coefficient drift, which more look-ahead does not remove. The
metric also reproduces the ordering across lines: 1007.02, whose metric is
the weakest at every window length (\SI{19.3}{\meter} at \SI{300}{\second}
against \SIrange{6.7}{7.6}{\meter} for the other three), is the line with the
largest measured error at every lag. We take \SI{300}{\second} as the
default. Second, the causal column is flat: across a
