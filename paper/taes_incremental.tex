...
A projected-gradient separability metric, derived from a condition that does
not require the redundant compensation coefficients to be identified, falls by
...
improve consistency under nonlinearity. The invariant line has reached this
application: Hager et al.\ formulate map-matching aiding as an error-state
filter on the $SE_2(3)$ group of extended poses, correcting the inertial
solution in closed loop so that the linearized dynamics stay accurate even for
a tactical-grade unit~\cite{hager2025lie}. The group structure improves the
propagation and the closed-loop error dynamics, but it does not directly
resolve the position--compensation ambiguity studied here, and the map
sensitivity remains governed by the local spatial gradient; the study is in
simulation and assumes every magnetic source but the anomaly field to be
perfectly corrected beforehand, so the aircraft field never enters. Its
degradation on low-information maps is consistent with the map-information
floor formalized in Section~\ref{sec:obs}.
However, these estimators are causal:
...
compensation have occurred, and can diverge when the initial residual is large;
...
smoothing per se is not a contribution, and the incremental solver is standard
machinery used as published~\cite{kaess2012}: what is new is the formulation
that the solver is applied to, and what it makes possible. That the resulting
estimator runs comfortably in real time, at \SIrange{7.3}{9.0}{\milli\second}
per state for a \SI{300}{\second} lag at \SI{1}{\hertz} states, is reported as
a feasibility result rather than claimed as novelty.
...
\begin{enumerate}
  ...
  \item \textbf{A separability condition and a flight-dependent lag metric.}
        We give a necessary and sufficient local condition for separating
        horizontal position from the compensation subspace, one that does not
        require the redundant Tolles--Lawson coefficients to be uniquely
        identified, and turn it into a computable weakest-direction
        information metric. Evaluated on the flight lines, it explains where
        the accuracy gained by lengthening the lag comes from and which line
        is hardest, and it indicates the transition beyond which the measured
        error is no longer predominantly separability-limited.
  \item \textbf{Flight-data validation against reproduced baselines.} On
        uncompensated cabin magnetometers across eight cold-start cases
        spanning four lines, two flights, two maps and two altitude regimes,
        under one untuned configuration, the smoothed estimate is the most
        accurate of the compared estimators on all eight cases and the causal
        estimate on six. The baselines are run rather than cited: the online
        ...
        component. Because a fixed-lag smoother produces
        a causal and a smoothed estimate and comparisons in the literature
        rarely distinguish them, we report both throughout, so that the
...
magnetic case~\cite{canciani2016,canciani2017}. Operationally the batch form
is a fix-taking scheme rather than a continuous one: a profile is collected
over a pass, correlated against the stored map to yield one position fix at
the end of that pass, and the inertial solution coasts until the next
one~\cite{golden1980}. A long flight is a sequence of such fixes. Three
properties of that scheme are worth carrying forward. The fix is available
only after the profile that produced it, so a delay of the profile length is
intrinsic to map matching and not something a smoother introduces. Successive
correlations are independent, so what one pass learned is not carried into the
next. And Eq.~\eqref{eq:inshard} makes the inertial increment exact within a
pass, which is why the recursive alternatives~\cite{hostetler1983} were
...
\label{eq:tlbasis}
\end{equation}
with $B_k$ in nanotesla, optionally augmented by a constant term, giving $m\le19$ coefficients.
...
This identity is exact in continuous time and is weakly violated by the
finite-differenced fluxgate rates. The induced columns are tied differently:
with normalized direction cosines $B_k(u^2+v^2+w^2)=B_k$ holds sample by
sample exactly, so their sum equals the total-field column, which is
collinear with the constant bias column only up to the variation of $B_k$,
about one percent RMS over a line. Both ties survive on the data, since with
every
...
makes the condition gauge-invariant by construction: the projector is
unchanged by any reparameterization of the coefficients that preserves the
compensation column space. Numerically, the metric below is unchanged across
four decades of rank tolerance despite the near-null directions. The FOGM disturbance $S_k$ of
...
static-coefficient reading of the model. It is a bound for that idealized single-window model and not for the
smoother, and its assumptions cut both ways. It is optimistic in holding $\delta\mathbf{p}$
...
nonlinear factors the carried prior is a Laplace approximation, a potential
source of overconfidence that the simulation of
Section~\ref{sec:consistency} finds below its resolution. This is the
precise sense in which a smoothed
...
depend on it very differently. Marginalization does not discard measurements,
it compresses them, so the causal estimate at epoch $k$ conditions on the same
information, every measurement up to $k$, whatever the lag is. Lengthening the
lag therefore should not improve it, and the only residual effect is that
retained states may be relinearized more often. What the lag does change is
how long a state stays revisable before Eq.~\eqref{eq:schur} freezes it: the
smoothed value for epoch $k-\ell$ conditions on measurements up to $k$, which
the causal value for that epoch could not have used. The smoothed output is
therefore not a delayed copy of the causal one but an estimate of a past epoch
made with $L$ seconds of hindsight, and the gap between the two is the price
of requiring an answer immediately. Section~\ref{sec:lagdesign} measures both
dependences and finds them as described here.
...
samples are largely redundant, and denser sampling cannot substitute for a
longer window: keeping all ten samples per state multiplies the rows of
$\mathbf{G}$ and $\boldsymbol{\Psi}$ without changing the path they trace, so
$\mu$ of Eq.~\eqref{eq:mu} improves by exactly the $\sqrt{10}$ of independent
averaging and its geometry not at all. That factor is not realized in flight,
because at \SI{67}{\meter\per\second} consecutive samples are under
\SI{7}{\meter} apart and share their map error; the ablation of
Section~\ref{sec:ablation} measures the outcome as a loss of
\SI{2.3}{\meter} on Mag~4 at four times the computation. Separability is
bought with track, not with samples.
...
baselines, which also use no future data, and it is the number a navigator
reads in flight; the smoothed value is the reconstructed trajectory, available
$L$ seconds after the fact, which is the quantity that matters wherever the
answer is consumed after the epoch it describes: survey and map products,
which are post-processed in any case, retrospective integrity checks on the
causal solution, and any supervisory loop that can accept the latency. The
look-ahead behind every reported number is stated in its table. One configuration
...
proposed estimator; the identically modelled online-TL EKF shares its
compensation prior $\sigma_{\beta}=100$, so that the margin between the two
cannot be attributed to the prior, while the NN-augmented filter and the
particle filter run the configurations stated below.
...
extended Kalman filter~\cite{gnadt2022}. It shares every model term and the
compensation prior with the factor graph, so the margin between the two
isolates the estimator structure, recursive commitment against windowed
re-linearization, from the modelling. The shared prior matters to the filter
far more than to the smoother: at the reference implementation's
calibrated-installation prior ($\sigma_{\beta}=1$) the filter diverges on two
Mag~4 lines and leaves the map on a third, the same prior sensitivity
diagnosed for the particle filter below, and at $\sigma_{\beta}=100$ it is
bounded on all eight cases, the values reported in
Table~\ref{tab:breadth}.
...
transient). The tuned filter uses the permanent TL group with an eight-unit hidden
layer. It is the one randomly initialized estimator in the comparison, so we
quantify its seed spread on 1007.06: over ten seeds it reaches a median of
\SI{43.8}{\meter} (interquartile range \SIrange{42.7}{45.5}{\meter}) on
Mag~4 and \SI{18.2}{\meter} (\SIrange{17.7}{18.5}{\meter}) on Mag~5,
consistent with published cold-start operating
points~\cite{gnadt2022,hager2026}; the tables quote one fixed-seed run per
case (\num{48.8}/\SI{18.3}{\meter} on this line).
...
in its coefficients, $\boldsymbol{\beta}$ is conditionally linear-Gaussian, which
motivates a Rao--Blackwellized implementation carrying the compensation and
the remaining approximately linear states in conditional Kalman blocks: ours carries
...
\textbf{Proposed, fixed-lag smoothed (\SI{300}{\second})} & \textbf{24.2} & \textbf{11.5} & \SI{300}{\second} \\
\bottomrule
\end{tabular}
\end{table}
...
online-TL EKF (\num{46.3}/\SI{17.6}{\meter}) and of the NN-augmented filter
...
\caption{North and east error histories, line 1007.06 Mag~4. The EKF+TL+NN is a fresh run (\SI{40.0}{\meter} here; ten-seed median \SI{43.8}{\meter}).}
\label{fig:latlon}
\end{figure}
...
\begin{table}[t]
\caption{Breadth: DRMS [m]; bold = best overall.}
\label{tab:breadth}
\centering
...
advantage is not statistically established (sign test $p=0.14$); it trails the EKF+TL+NN on 1007.02 Mag~5 by \SI{2.7}{\meter}
and on 1003.08 Mag~4 by \SI{2.3}{\meter}, and we report both rather than
claiming a sweep. The margin is widest where the lines are hardest: on the
two Mag~4 cases where the shared-prior EKF reaches \SI{87.5}{} and
\SI{155.9}{\meter}, the smoothed estimate reads \SI{28.4}{} and
\SI{19.6}{\meter}. The mechanism is the one Section~\ref{sec:sysmodel}
anticipated: the filter must explain the uncompensated field with whatever
state is cheapest at each instant and commits immediately, while the
smoother may revise its attribution for \SI{300}{\second} before committing;
and the filter's dependence on the prior scale
(Section~\ref{sec:baselines}) is the recursive face of the same coin,
since a one-step update has no window across which to amortize a mis-scaled
prior.
...
\begin{table}[t]
\caption{Lag sweep: smoothed DRMS [m], with the causal DRMS at the shortest and longest lag.}
\label{tab:lag}
\centering
\setlength{\tabcolsep}{4pt}
\begin{tabular}{ll rrrrr rr}
\toprule
 & & \multicolumn{5}{c}{lag $L$ [s], smoothed} & \multicolumn{2}{c}{causal} \\
\cmidrule(lr){3-7}\cmidrule(l){8-9}
Line & Mag & 30 & 60 & 150 & 300 & 600 & $L{=}30$ & $L{=}300$ \\
\midrule
1007.06 & 4 & 40.2 & 37.1 & 29.7 & 24.2 & 21.9 & 43.5 & 42.7 \\
...
buys between \SI{3.6}{} and \SI{41.8}{\meter}. The knee separates two regimes,
and the separability metric of Eq.~\eqref{eq:mu} says which is which:
Fig.~\ref{fig:obsmetric} evaluates $\mu^{-1/2}$ in sliding windows of length
equal to the lag, at the \SI{1}{\hertz} kept cadence. Being a single-window quantity, the metric does
...
is the line with the largest measured error at every lag on both sensors. The
metric therefore explains the pre-knee regime and ranks the relative
difficulty of the lines, but it is not by itself a predictor of the optimal
lag, which is set by where the separability-limited regime ends. We take
\SI{300}{\second} as the default. Second, the causal columns are nearly flat:
from \SI{60}{\second} upward the causal error moves by at most
...
than separability requires and the compensation inside it is poorly determined
for the newest state as well as the old ones. Above that threshold the
flatness is the behaviour Section~\ref{sec:incremental} anticipates, and it
is a check on the marginalization rather than a disappointment. Because the
states leaving the lag are compressed into the marginal prior of
Eq.~\eqref{eq:schur} rather than dropped, the newest state conditions on the
whole flight whatever the lag is, so only the retrospective revision has
anything to gain from a longer window. The contrast with a sliding window that
discards its history is instructive: in the GNSS/INS comparison
of~\cite{wen2021}, where the graph retains only the states inside the window,
the current-epoch error improves steadily as the window grows, from
\SI{5.18}{\meter} at \SI{1}{\second} to \SI{3.65}{\meter} at the batch limit,
because there the window length decides how much information the estimate has
at all. Here it decides only how long a state is revised before it is frozen,
...
the four columns rather than the smoothed column alone. The compensation prior may
move an order of magnitude in either
...
\SI{9.8}{\meter} causal on Mag~5. Removing the Huber kernel or widening its threshold costs one to
...
demonstrate a plateau. Attaching all ten raw
...
do so by leaning on the tight anchor, and the advantage disappears with it. The information
...
Over \num{40} independent runs the causal average NEES is $2.10\pm0.13$ and
the smoothed average $2.11\pm0.18$, mean and 95\% $t$-interval over run
...
chi-square band, is the honest error bar. Both intervals contain \num{2}:
the solver machinery, including the marginalization of
Eq.~\eqref{eq:schur}, is statistically consistent when the model is true,
and any overconfidence from the frozen linearizations is below the
resolution of the test, under about ten percent in the NEES (variance)
sense. The test exercises the Gaussian specialization; the robust kernel of
the flown configuration is outside its scope.
...
\SI{1}{\hertz} state cadence exceeds two orders of magnitude. The per-epoch
cost is set by the number of retained states and not by elapsed time, so a
flight of several hours costs the same per update as the lines measured here;
what we demonstrate is the cost model, on lines of \SIrange{63}{87}{\minute},
not an hours-long endurance run. The cost scales
...
smaller and not statistically established on eight cases. The lag sweep locates the design knee
near \SI{300}{\second}, about \SI{20}{\kilo\meter} of track. In the tested
cases, lengthening the lag acts almost purely as a purchase of smoothing
accuracy with latency, since the causal DRMS varies by less than a metre
across the sweep. The ablation finds no delicate modelling
parameter, apart from the numerical process-noise floor that must be kept
minimal: one configuration serves every line and both sensors, and the update
...
counts across cases, not distributions. The reported covariance is consistent
under matched-model Monte Carlo simulation, but the flight coverage
diagnostic shows substantial overconfidence where unmodeled map error and
residual interference dominate, most of all for the smoothed Mag~4 output, so
the covariance should not be used as an integrity bound as it stands. The
...
These limitations set the agenda: robust covariance calibration and integrity
monitoring under model mismatch, a transit-like flight regime, sparser
instantiation of the compensation chain in the manner of~\cite{park2024}, and
a native-factor implementation that would carry the real-time margin from the
state cadence down to the raw sensor rate. A further direction composes
naturally with this work rather than competing with it: the inertial chain
could be carried on the group of extended poses in the manner
of~\cite{hager2025lie}, whose benefit is that the propagation Jacobians stop
depending on the estimate. That is attractive here for a reason specific to a
smoother, since it is at the marginalization boundary, where
Eq.~\eqref{eq:schur} freezes a linearization, that estimate-dependent
Jacobians do the most damage, and the flight coverage of
Section~\ref{sec:consistency} is where such a change would show up.
...
received the M.S. and Ph.D. degrees in aerospace engineering from the Korea
Advanced Institute of Science and Technology (KAIST), Daejeon, Republic of Korea,
in 2021 and 2025, respectively. He is currently a postdoctoral researcher at KAIST.
His research interests include GNSS-denied navigation, magnetic-anomaly navigation,
fault diagnosis and fault-tolerant control, and data-driven estimation and control
for autonomous aerospace systems.
\end{biography}

\begin{biography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figs/author_chang.png}}]{Bohyun Chang}
received the M.S. degree in aerospace engineering from the Korea Advanced Institute
of Science and Technology (KAIST), Daejeon, Republic of Korea. He is currently
pursuing the Ph.D. degree in aerospace engineering at KAIST. His research interests
include guidance, navigation, and control of aerospace vehicles.
\end{biography}
...
Republic of Korea, as a professor. His research interests include spacecraft guidance,
